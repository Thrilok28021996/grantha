...
import { basename, resolveTarget, stripMd } from '../lib/links'
...
// Extracted helpers — pure functions and autocomplete logic, kept out of this 2k-line file.
import { caretOffsetAt, caretLine, orderedLabel, tagAvatarColor, sameStrings } from './editorUtils'
import { WRAP_PAIRS, buildAcItems, buildSupertagMap, type AcState } from './autocomplete'
...
export function BlockEditor({
  ...
  const supertagIndex = useMemo(() => buildSupertagMap(parsed), [parsed])
  ...
  const openSpellMenu = (blockId: number, word: string, x: number, y: number, at = -1): void => {
    ...
    void window.grantha.suggestSpelling(word).then((suggestions) =>
      setSpellMenu((prev) =>
  ...
  const ignoreSpellWord = (word: string): void => {
    void window.grantha.addToDictionary(word).then(() => resetSpell())
    setSpellMenu(null)
  }
  ...
  const smartTitlePaste = (blockId: number, url: string): void => {
    ...
    void window.grantha.fetchTitle(url).then((title) => {
      if (!title) return
  ...
  const handleFiles = async (files: FileList, afterId: number): Promise<void> => {
    ...
    for (const file of Array.from(files)) {
      try {
        const rel = await window.grantha.saveAsset(file.name, await fileToBase64(file))
  ...
  const acItems = (state: AcState): AcSuggestion[] =>
    buildAcItems(state, files, templates, parsed, index, supertagIndex)
  ...
  const insertTemplate = async (id: number, tplPath: string): Promise<void> => {
    ...
    let raw: string
    try {
      raw = (await window.grantha.readNote(tplPath))?.text ?? ''
    } catch (err) {
