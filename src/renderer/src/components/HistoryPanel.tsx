...
export function HistoryPanel({ path, onClose }: { path: string; onClose: () => void }): React.JSX.Element {
  ...
  useEffect(() => {
    void window.grantha.listSnapshots(path).then((list) => {
      setSnaps(list)
  ...
  useEffect(() => {
    if (!sel) return
    void window.grantha.readSnapshot(path, sel).then((t) => setPreview(t ?? ''))
  }, [path, sel])
      ...
      <div className="modal history-modal" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {snaps === null ? (
          ...
          <p className="empty-note">
            No snapshots yet. Grantha keeps one before overwriting a note (at most every 10 minutes) —
