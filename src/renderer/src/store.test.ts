 ...
 *
 * The store touches `window.grantha` and `localStorage` at module scope, so both
...
describe('per-path write queue', () => {
  ...
  it('surfaces a failed write as saveError instead of swallowing it', async () => {
    seed({ 'a.md': 'a' })
    grantha.writeNote.mockResolvedValueOnce({ ok: false, error: 'ENOSPC: disk full' })
    const st = useStore.getState()
    st.editNote('a.md', 'a2')
    await st.saveActive()
  ...
  it('re-queues a failed write so the next flush retries it', async () => {
    ...
    expect(grantha.writeNote).toHaveBeenCalledTimes(2)
...
describe('renameNote', () => {
  ...
  it('retargets canvas file cards pointing at the old path', async () => {
    ...
    grantha.readCanvas.mockResolvedValueOnce({
      nodes: [
    ...
    await useStore.getState().renameNote('Old.md', 'New')
    expect(grantha.writeCanvas).toHaveBeenCalledTimes(1)
    const doc = grantha.writeCanvas.mock.calls[0][1] as { nodes: { file?: string }[] }
...
describe('streaming vault hydration', () => {
  ...
  it('reads the note being shown on its own, ahead of the batches', async () => {
    ...
    const asked = grantha.readNotesCached.mock.calls.flatMap((c: unknown[]) => c[0] as string[])
  ...
  it('never clobbers a note edited while the rest of the vault streams in', async () => {
    ...
    let edited = false
    grantha.readNotesCached.mockImplementation(async (paths: string[]) => {
      if (!edited) {
        edited = true
