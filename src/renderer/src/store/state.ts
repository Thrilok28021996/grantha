...
import type { CanvasMeta, FileEvent, NoteFile, ParsedNote, Workspace } from '@shared/types'
...
import type { ViewMode, HistEntry, ThemeName, ModalKind, IndentGuides, SidePane } from './types'

// Two one-time migrations: releases before 0.2 used "inkwell", then "verso".
// Must run before the store's initial state reads any of them.
if (typeof localStorage !== 'undefined') {
  for (const k of Object.keys(localStorage)) {
    // Migrate inkwell-* or verso-* keys to grantha-*
    const prefix = k.startsWith('inkwell-') ? 'inkwell-' : k.startsWith('verso-') ? 'verso-' : null
    if (!prefix) continue
    const renamed = 'grantha-' + k.slice(prefix.length)
...
async function loadWorkspaceBases(): Promise<Base[]> {
  const vault = normalizeBases(await window.grantha.readBases())
  ...
  await window.grantha.writeBases(merged)
  return merged
}

export const useStore = create<GranthaState>((set, get) => {
  ...
  const queueWrite = (p: string, t: string): Promise<void> => {
    const chained = (writeChains.get(p) ?? Promise.resolve()).then(async () => {
      const res = await window.grantha.writeNote(p, t)
  ...
  const hydrateVault = async (gen: number, paths: string[]): Promise<void> => {
    for (let i = 0; i < paths.length; i += HYDRATE_CHUNK) {
      ...
      const contents = await window.grantha.readNotesCached(slice)
      ...
      if (fresh.length) void window.grantha.saveParseCache(fresh)
  ...
  const applyWorkspace = async (ws: Workspace): Promise<void> => {
    ...
    const home = localStorage.getItem('grantha-home') !== 'last'
    ...
    if (first) {
      const content = await window.grantha.readNote(first)
      if (content !== null) {
    ...
    const canvases = await window.grantha.listCanvases()
    if (gen !== loadGen) return
  ...
  const ensureLoaded = async (path: string): Promise<void> => {
    ...
    const content = await window.grantha.readNote(path)
  ...
  return {
    ...
    modal: null,
    paletteOpen: false,
    sidebarOpen: localStorage.getItem('grantha-sidebar') !== 'closed',
    rightbarOpen: localStorage.getItem('grantha-rightbar') !== 'closed',
    sidebarWidth: clampPanel(Number(localStorage.getItem('grantha-sidebar-w')) || 260, 'sidebar'),
    rightbarWidth: clampPanel(Number(localStorage.getItem('grantha-rightbar-w')) || 280, 'rightbar'),
    ...
    zen: false,
    theme: (localStorage.getItem('grantha-theme') as ThemeName | null) ?? 'dark',
    accent: localStorage.getItem('grantha-accent') ?? 'indigo',
    customCss: null,
    // Same Paper-implies-Serif default as setTheme, applied on boot.
    editorFont:
      localStorage.getItem('grantha-font') ??
      (localStorage.getItem('grantha-theme') === 'paper' ? 'serif' : 'sans'),
    editorFontSize: Number(localStorage.getItem('grantha-fontsize')) || 16,
    indentGuides: (localStorage.getItem('grantha-guides') as IndentGuides | null) ?? 'plain',
    spellcheck: localStorage.getItem('grantha-spellcheck') !== 'off',
    smartLinkTitles: localStorage.getItem('grantha-smart-titles') !== 'off',
    homeJournal: localStorage.getItem('grantha-home') !== 'last',
    dirty: false,
    loading: false,
    saveError: null,
    ...
    setTheme: (theme) => {
      localStorage.setItem('grantha-theme', theme)
      ...
      const chosen = localStorage.getItem('grantha-font')
      const editorFont = chosen ?? (theme === 'paper' ? 'serif' : 'sans')
      set({ theme, editorFont })
    },

    setAccent: (accent) => {
      localStorage.setItem('grantha-accent', accent)
      set({ accent })
    },

    reloadCustomCss: async () => {
      set({ customCss: await window.grantha.readCustomCss() })
    },
    ...
    toggleSidebar: () => {
      ...
      localStorage.setItem('grantha-sidebar', sidebarOpen ? 'open' : 'closed')
      set({ sidebarOpen })
    },

    toggleRightbar: () => {
      const rightbarOpen = !get().rightbarOpen
      localStorage.setItem('grantha-rightbar', rightbarOpen ? 'open' : 'closed')
      set({ rightbarOpen })
    },

    setSidebarWidth: (px) => {
      const sidebarWidth = clampPanel(px, 'sidebar')
      localStorage.setItem('grantha-sidebar-w', String(sidebarWidth))
      set({ sidebarWidth })
    },

    setRightbarWidth: (px) => {
      const rightbarWidth = clampPanel(px, 'rightbar')
      localStorage.setItem('grantha-rightbar-w', String(rightbarWidth))
      set({ rightbarWidth })
    },
    ...
    setEditorFont: (editorFont) => {
      localStorage.setItem('grantha-font', editorFont)
      set({ editorFont })
    },

    setIndentGuides: (indentGuides) => {
      localStorage.setItem('grantha-guides', indentGuides)
      set({ indentGuides })
    },

    setSpellcheck: (on) => {
      localStorage.setItem('grantha-spellcheck', on ? 'on' : 'off')
      set({ spellcheck: on })
    },

    setEditorFontSize: (editorFontSize) => {
      localStorage.setItem('grantha-fontsize', String(editorFontSize))
      set({ editorFontSize })
    },

    setHomeJournal: (homeJournal) => {
      localStorage.setItem('grantha-home', homeJournal ? 'journal' : 'last')
      set({ homeJournal })
    },

    setSmartLinkTitles: (smartLinkTitles) => {
      localStorage.setItem('grantha-smart-titles', smartLinkTitles ? 'on' : 'off')
      set({ smartLinkTitles })
    },
    ...
    newFromTemplate: async (templatePath, folder = '') => {
      const tpl = (await window.grantha.readNote(templatePath))?.text ?? ''
      ...
      const created = await window.grantha.createNote(path, text)
      if (created) {
    ...
    ensureEntity: async (name, tag) => {
      ...
      let path = existing
      if (!path) {
        const created = await window.grantha.createNote(pathForNewNote(name), '')
    ...
    createSupertag: async (name, opts) => {
      ...
      const created = await window.grantha.createNote(path, '---\nfields: {}\n---\n')
    ...
    createSupertagFromFolder: async (folder) => {
      ...
      if (existing) {
        ...
        const created = await window.grantha.createNote(tagPath, text)
        if (!created) return
        await get().applyFileEvent({ type: 'add', file: created })
      }
    ...
    applyTemplateToNote: async (notePath, templatePath) => {
      const raw = (await window.grantha.readNote(templatePath))?.text ?? ''
    ...
    setBases: (bases) => {
      void window.grantha.writeBases(bases)
    ...
    createCanvas: async (name, folder = '') => {
      ...
      const created = await window.grantha.createCanvas(path)
      if (!created) return
    ...
    deleteCanvas: async (path) => {
      const ok = await window.grantha.deleteCanvas(path)
      if (!ok) return
    ...
    renameCanvas: async (path, name) => {
      ...
      const meta = await window.grantha.renameCanvas(path, newPath)
      if (!meta) return
      set({
    ...
    bootstrap: async () => {
      set({ recentVaults: await window.grantha.getWorkspaces() })
      const last = await window.grantha.getLastWorkspace()
      if (!last) return
      set({ loading: true })
      try {
        const ws = await window.grantha.loadWorkspace(last)
    ...
    openWorkspace: async () => {
      set({ loading: true })
      try {
        const ws = await window.grantha.openWorkspace()
        if (ws) await applyWorkspace(ws)
      } finally {
        set({ loading: false })
      }
    ...
    openDemoVault: async () => {
      set({ loading: true })
      try {
        const ws = await window.grantha.openDemoVault()
        if (ws) await applyWorkspace(ws)
      } finally {
        set({ loading: false })
      }
    ...
    switchVault: async (root) => {
      ...
      set({ loading: true })
      try {
        const ws = await window.grantha.loadWorkspace(root)
        if (ws) await applyWorkspace(ws)
      } finally {
        set({ loading: false })
      }
    ...
    forgetVault: async (root) => {
      const remaining = await window.grantha.forgetWorkspace(root)
    ...
    reloadVault: async () => {
      ...
      if (!ws) return
      const fresh = await window.grantha.loadWorkspace(ws.root)
      if (!fresh) return
      const gen = ++loadGen
      ...
      const parsed: Record<string, ParsedNote> = {}
      if (keep) {
        const content = await window.grantha.readNote(keep)
        if (content !== null) {
    ...
    ensureDailyNote: async (iso) => {
      ...
      if (!exists || empty) {
        ...
        const seed = tpl
          ? applyTemplate((await window.grantha.readNote(tpl.path))?.text ?? '', basename(path), new Date())
          : ''
        if (!exists) {
          const created = await window.grantha.createNote(path, seed)
    ...
    applyFileEvent: async (event) => {
      ...
      if (evPath === '.verso/bases.json') {
        const bases = normalizeBases(await window.grantha.readBases())
        const activeBaseId = bases.some((b) => b.id === get().activeBaseId)
          ? get().activeBaseId
          : (bases[0]?.id ?? null)
        set({ bases, activeBaseId })
      ...
      if (event.type === 'rename') {
        ...
        const content = await window.grantha.readNote(event.path)
        set((state) => {
          const texts = { ...state.texts }
          const parsed = { ...state.parsed }
      ...
      const content = await window.grantha.readNote(file.path)
      if (content === null) return
    ...
    renameNote: async (oldPath, input) => {
      ...
      await flushAll()
      const created = await window.grantha.renameNote(oldPath, newPath)
      if (!created) {
      ...
      try {
        ...
        for (const c of get().canvases) {
          const doc = normalizeDoc(await window.grantha.readCanvas(c.path))
          let changed = false
          ...
          if (changed) await window.grantha.writeCanvas(c.path, { ...doc, nodes })
        }
      } catch (e) {
    ...
    deleteNote: async (path) => {
      const ok = await window.grantha.deleteNote(path)
      if (!ok) {
    ...
    duplicateNote: async (path) => {
      const text = get().texts[path] ?? (await window.grantha.readNote(path))?.text ?? ''
      const base = path.replace(/\.md$/i, '')
      const taken = new Set(get().files.map((f) => f.path.toLowerCase()))
      let np = `${base} copy.md`
      let i = 2
      while (taken.has(np.toLowerCase())) np = `${base} copy ${i++}.md`
      const created = await window.grantha.createNote(np, text)
      if (created) {
        await get().applyFileEvent({ type: 'add', file: created })
        get().openNote(created.path)
      } else {
    ...
    revealNote: async (path) => {
      await window.grantha.revealNote(path)
    }
  }
})
