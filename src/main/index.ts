...
// Must run before app is ready: lets `grantha://` load in <img> and via fetch.
...
function prefsPath(): string {
  return path.join(app.getPath('userData'), 'grantha-prefs.json')
}
...
function createWindow(): void {
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // dictionaries from a Google CDN — Grantha's own bundled nspell pipeline already
  ...
  ipcMain.handle('app:checkUpdates', async () => {
    try {
      ...
      const res = await net.fetch('https://api.github.com/repos/ed-nico/verso_notes/releases/latest', {
        signal: ctrl.signal,
        headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'Grantha' }
      })
      clearTimeout(timer)
    ...
    try {
      ...
      for (let hop = 0; hop < 5; hop++) {
        ...
        res = await net.fetch(target.href, {
          signal: ctrl.signal,
          redirect: 'manual',
          headers: { 'User-Agent': 'Mozilla/5.0 Grantha' }
        })
...
function applyCsp(): void {
  const devUrl = process.env.ELECTRON_RENDERER_URL
  const policy = devUrl
    ? "default-src 'self' 'unsafe-inline' 'unsafe-eval' data: blob: grantha: https: ws: http://localhost:*; worker-src 'self' blob:"
    : PROD_CSP
  ...
  const isAppUrl = (url: string): boolean => {
    try {
      const u = new URL(url)
      if (u.protocol === 'file:' || u.protocol === 'grantha:') return true
  ...
  session.defaultSession.webRequest.onBeforeSendHeaders(
    ...
    (details, cb) => {
      cb({ requestHeaders: { ...details.requestHeaders, Referer: 'https://grantha.app/' } })
    }
  )
}

/** Serve workspace files to the renderer via grantha://asset/<relative-path>. */
function registerAssetProtocol(): void {
  protocol.handle('grantha', async (request) => {
    try {
      const url = new URL(request.url)
      // Only the `asset` host is a thing; don't serve grantha://anything-else/….
      ...
      // The renderer's origin isn't grantha://, so cors-mode fetch() consumers
