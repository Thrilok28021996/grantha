 ...
 *  the watcher, and the `grantha://` protocol. Defined once in `shared/media.ts`
...
const GRANTHA_WATCHED = new Set(['.verso/bases.json', '.verso/custom.css'])
...
function isWatchedFile(root: string, abs: string): boolean {
  const rel = toRelative(root, abs)
  if (GRANTHA_WATCHED.has(rel)) return true
...
async function startWatching(root: string, win: BrowserWindow): Promise<void> {
  ...
  watcher = chokidar.watch(root, {
    ignored: (p: string) => {
      ...
      if (rel === '.verso' || GRANTHA_WATCHED.has(rel)) return false
