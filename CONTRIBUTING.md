...
Thanks for your interest! Grantha is a local-first Markdown notebook built with
...
exposed as `window.grantha`) into the main process (`src/main/index.ts` for IPC and
...
1. `src/shared/types.ts` — add it to `GranthaApi` (the cross-process contract)
...
- All renderer-initiated disk writes go through the store's `queueWrite` — never
  call `window.grantha.writeNote` directly from store actions.
- Assets shown in notes are served through the `grantha://` protocol; the renderer
