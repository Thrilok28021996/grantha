...
- **💬 Quotes & callouts** — `>` for a blockquote; `> [!warning] Careful` for an Obsidian-style callout in a dozen kinds. Add `-` (`[!tip]-`) to start it collapsed.
- **🧮 Math** — `$E = mc^2$` inline and `$$…$$` for display, rendered with KaTeX. Prices stay prices: "it costs $5 and $10" is left alone.
...
- **macOS** (Apple Silicon) — download the `.dmg` and drag Grantha into Applications.
  ...
  ```bash
  xattr -cr /Applications/Grantha.app
  ```

  …or build from source below, which needs no workaround.
- **Linux** (x64) — download the `.AppImage`, `chmod +x` it, and run.
- **Windows** (x64) — download and run `Grantha-Setup-*.exe`. The installer is unsigned,
...
Grantha has **no telemetry, no analytics, no accounts, and no update pings**, and it never opens a network port. 
...
Grantha is in active development, so expect rough edges and the occasional breaking change. Issues and ideas welcome.
...
Full transparency: this app was largely written with AI assistance. I use Grantha daily as my own notes app, and it works well for my needs — but treat it accordingly: keep backups of your vault (your notes are plain Markdown files, so any backup or sync tool works), and review the code at your free will — it's all here.

## License

[MIT](LICENSE). The code is free to use, modify, and redistribute; the **Grantha** name and logo are not covered by the license — please ship forks under a different name.
