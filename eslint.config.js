...
export default tseslint.config(
  ...
  // ---- main process (Node / Electron) ----
  {
    ...
    // No react-hooks here — this is pure Node code.
    rules: {
      '@typescript-eslint/no-unused-vars': [
        'error',
        { argsIgnorePattern: '^_', varsIgnorePattern: '^_', caughtErrors: 'none' }
      ...
      '@typescript-eslint/no-explicit-any': 'error',
      'no-empty': ['error', { allowEmptyCatch: true }],
      // The main process must never shell out to untrusted input.
      'no-restricted-syntax': [
        'error',
        {
          selector: 'CallExpression[callee.property.name="execSync"]',
          message: 'Use execFile or spawn (with args array) instead of execSync to avoid shell injection.'
        },
        {
          selector: 'CallExpression[callee.property.name="exec"]',
          message: 'Use execFile or spawn (with args array) instead of exec to avoid shell injection.'
        }
      ]
    }
  }
