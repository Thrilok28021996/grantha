...
**Naming note:** everything is **grantha** — the preload bridge is `window.grantha` (typed
`GranthaApi`), the asset protocol is `grantha://`, prefs live in `grantha-prefs.json`. Two
...
npm run build:mac   # build + electron-builder → dist/Grantha-<ver>-arm64.dmg (Apple Silicon, ad-hoc signed)
...
```
renderer (React)  ──window.grantha──▶  preload (contextBridge)  ──ipcRenderer.invoke──▶  main
   src/renderer/                          src/preload/index.ts                    src/main/{index,workspace}.ts
```

- **`src/shared/types.ts`** — the contract between processes. `GranthaApi` is the full IPC
...
- **`src/main/index.ts`** — window creation, IPC handlers, CSP (permissive in dev, strict in
  prod), and the `grantha://` custom protocol that serves workspace assets (images/PDFs) to
  the renderer. Remembers the last workspace in `userData/grantha-prefs.json`.
...
- **All disk writes go through `queueWrite`** — a per-path promise chain, so a debounced
  ...
  `window.grantha.writeNote` directly from the store.
...
- **Per-vault data lives in the vault, not localStorage.** Bases are stored in
  ...
  a one-time migration from the old `grantha-bases` localStorage key. Don't reintroduce
...
- Adding an IPC call means touching four files in order: `shared/types.ts` (`GranthaApi`) →
...
- Assets referenced in notes must be served through `grantha://` (handled in `main/index.ts`);
